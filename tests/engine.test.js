...
eq(getCheckout(52, 'single')?.path, 'T17 S1', '52 SO: kein 1-Dart-Feld=52 — jetzt echter 2-Dart-Pfad statt Platzhalter (Bug vorher: fälschlich "S52", danach nur "setup + finish")');
eq(getCheckout(23, 'single')?.path, 'D11 S1', '23 SO: in einem Dart nicht erreichbar (kein S/D/T ergibt 23) — echter 2-Dart-Pfad statt Platzhalter');

eq(getCheckout(52, 'single')?.darts, 2, '52 SO 2 darts');
eq(getCheckout(100, 'single')?.path, 'T20 D20', '100 SO 2-dart');
eq(getCheckout(121, 'single')?.path, 'T20 T20 S1', '121 SO 3-dart, in 2 Darts nicht erreichbar');
eq(getCheckout(178, 'single'), null, '178 echter Bogey, auch Single Out unmöglich');
eq(getCheckout(23, 'single', 1)?.path, undefined, '23 mit nur 1 Dart übrig unerreichbar');
// Regression: Pfadsuche muss echte Mindest-Dart-Anzahl finden, nicht am größten
// ersten Dart (meist T20) festhalten, wenn der dadurch nur in 3 statt 2 Darts fertig wird.
eq(getCheckout(97, 'single')?.path, 'T19 D20', '97 SO: kürzester Pfad (2 Darts), nicht T20-first mit 3 Darts');
eq(getCheckout(97, 'single')?.darts, 2, '97 SO 2 darts, nicht 3');
